...
We end up with finished games of Go, with which we can update the network. 
...
This process is repeated over and over again and reminds of a method called policy iteration, which is a technique for finding optimal policies in Markov decision processes. 
However, instead of using a greedy policy improvement, AlphaGo Zero used MCTS to select actions that were better than those suggested by the raw network. 
This way, AlphaGo Zero makes use of a improved lookahead version of the raw network.
...
Previously, the policy network of AlphaGo was trained on a large dataset of human expert moves. 
In AlphaGo Zero it was trained by playing 29 million games against itself, becoming its own teacher. 
The policy network still outputs a probability distribution over all possible moves for a given board position. 
Instead of predicting the moves that human experts played, the network is now trained to predict the moves that AlphaGo Zero played in the self-play games,  
...
AlphaGo and AlphaZero have inspired human players to learn from their moves, and to improve their own skills and understanding.
