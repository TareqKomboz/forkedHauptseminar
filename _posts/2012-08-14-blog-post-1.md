...
<figure>
  <img src="https://tareqkomboz.github.io/forkedHauptseminar/images/PolicyNetwork.png" alt="Image of Policy Network"/>
  <figcaption>Source: Silver, David et al., 2017</figcaption>
</figure>

<figure>
  <img src="https://tareqkomboz.github.io/forkedHauptseminar/images/ValueNetwork.png" alt="Image of Value Network"/>
  <figcaption>Source: Silver, David et al., 2017</figcaption>
</figure>

# Training procedure

The training of AlphaGo involved two stages: supervised learning and self-play via reinforcement learning. 
...
In the self-play stage, where AlphaGo played millions of games against itself, this policy was further improved by reinforcement learning, promoting moves that lead to successes and surpressing moves that lead to failures. 
...
2. Expansion: The leaf node is expanded by adding all possible child nodes to the tree.
4. Simulation: A simulation is performed by using the policy network to select moves until the end of the game is reached. The result of the simulation is then used to update the search tree.
5. Backpropagation: The result of the simulation is backpropagated up the search tree to update the statistics of the nodes that were visited during the selection phase.

 At each node of the tree, AlphaGo uses its policy and value networks to evaluate the position and select the best move. The search tree is then expanded and updated with the results of the simulation. MCTS is used to guide the search for the best move by focusing on the most promising parts of the search tree. This makes the search more efficient and allows AlphaGo to find the best move in a reasonable amount of time.
...
AlphaGo Lee was the first computer program to defeat a world champion in Go by combining existing MCTS and Deep Learning.
AlphaGo Zero, which was trained from scratch without human data, outperforms every other alogorithm in the game of Go through self-learning only.
Simpler tree search algorithms and architectures can lead to better results.
